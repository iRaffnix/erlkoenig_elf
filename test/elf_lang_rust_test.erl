...
sym_le(StName, StInfo, StShndx, StValue, StSize) ->
    <<StName:32/little, StInfo:8, 0:8,
      StShndx:16/little, StValue:64/little, StSize:64/little>>.
...
make_rust_elf(SymNames, RodataContent, CommentContent) ->
    ...
    ShStrTab = <<0, ".text", 0, ".symtab", 0, ".strtab", 0,
                 ".rodata", 0, ".comment", 0, ".shstrtab", 0>>,
    ...
    SymEntries = lists:zipwith(
        ...
        end, NameOffsets, lists:seq(0, length(NameOffsets) - 1)),
    ...
    Header = elf_header_le(?ET_EXEC, ?EM_X86_64, ?TEXT_VADDR, 64,
                           ShdrOff, 1, NumSections, ShStrNdx),
    ...
    S1 = shdr_le(1, ?SHT_PROGBITS, ?SHF_ALLOC bor ?SHF_EXECINSTR,
    ...
    <<Header/binary, Phdr/binary,
      TextContent/binary, SymtabData/binary, StrTab/binary,
      RodataContent/binary, CommentContent/binary, ShStrTab/binary,
      Pad/binary,
      S0/binary, S1/binary, S2/binary, S3/binary,
      S4/binary, S5/binary, S6/binary>>.
...
rustc_comment() ->
    <<0, "GCC: (Debian 13.2.0-25) 13.2.0", 0,
      "rustc version 1.77.0 (aedd173a2 2024-03-17)", 0>>.
...
panic_strings_extract_crates_test() ->
    ...
    {ok, Elf} = elf_parse:from_binary(Bin),
    {ok, Info} = elf_lang_rust:parse(Elf),
    PanicCrates = [C || C <- Info#rust_info.crates,
...
parse_full_test() ->
    ...
    TokioCrates = [C || C <- Info#rust_info.crates,
