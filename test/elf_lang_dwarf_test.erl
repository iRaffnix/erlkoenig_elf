...
%%   terminated by (0,0)
make_debug_abbrev() ->
    <<1,                     %% abbrev code 1 (ULEB128)
      ?DW_TAG_compile_unit,  %% tag (ULEB128)
      1,                     %% has_children = yes
      ?DW_AT_producer, ?DW_FORM_strp,
      ?DW_AT_language, ?DW_FORM_data2,
      ?DW_AT_name, ?DW_FORM_strp,
      ?DW_AT_comp_dir, ?DW_FORM_strp,
      0, 0,                  %% end of attr specs
...
make_elf_with_dwarf(DebugInfo, DebugAbbrev, DebugStr, DebugLine) ->
    ...
    %% .shstrtab content
    ShStrTab = case HasLine of
        false ->
            <<0, ".debug_info", 0, ".debug_abbrev", 0,
              ".debug_str", 0, ".shstrtab", 0>>;
        true ->
            <<0, ".debug_info", 0, ".debug_abbrev", 0,
              ".debug_str", 0, ".debug_line", 0, ".shstrtab", 0>>
    end,
    ...
    DebugStrNameIdx = 27,
    {DebugLineNameIdx, ShstrtabNameIdx} = case HasLine of
        false -> {0, 38};
        true -> {38, 50}
    end,

    NumSections = case HasLine of false -> 5; true -> 6 end,
    ...
    {DlOff, DlSz, SsOff} = case HasLine of
        false ->
            {0, 0, DsOff + DsSz};
        true ->
            DlO = DsOff + DsSz,
    ...
    LineShdrs = case HasLine of
        false -> <<>>;
        true ->
            shdr_le(DebugLineNameIdx, 1, 0, 0, DlOff, DlSz, 0, 0, 1, 0)
    end,
    ...
    <<Header/binary,
      ...
      (case HasLine of false -> <<>>; true -> DebugLine end)/binary,
      ShStrTab/binary, Pad/binary,
      Shdr0/binary, Shdr1/binary, Shdr2/binary, Shdr3/binary,
      LineShdrs/binary, ShdrLast/binary>>.
...
uleb128_multibyte_test() ->
    ...
    AbbrevCode128 = <<128, 1>>,  %% ULEB128 for 128
    Abbrev = <<AbbrevCode128/binary,
               ?DW_TAG_compile_unit,
               1,  %% has_children
               ?DW_AT_producer, ?DW_FORM_strp,
               ?DW_AT_language, ?DW_FORM_data2,
               0, 0,
               0>>,
    Die = <<AbbrevCode128/binary,
            ?STR_OFF_PRODUCER:32/little,
            ?DW_LANG_Rust:16/little>>,
...
source_files_v4_test() ->
    ...
    Bin = make_elf_with_dwarf(make_debug_info_v4(), make_debug_abbrev(),
                              make_debug_str(), DebugLine),
    {ok, Elf} = elf_parse:from_binary(Bin),
...
language_decoding_test() ->
    Langs = [
        {16#0001, c89}, {16#0002, c}, {16#0004, cpp},
        {16#000C, c99}, {16#0016, go}, {16#001A, cpp11},
        {16#001C, rust}, {16#001D, c11}, {16#0021, cpp14},
        {16#002C, c17}, {16#FFFF, {unknown, 16#FFFF}}
    ],
    lists:foreach(fun({Code, Expected}) ->
        Die = <<1, ?STR_OFF_PRODUCER:32/little, Code:16/little,
                ?STR_OFF_NAME:32/little, ?STR_OFF_COMPDIR:32/little>>,
        CUHeader = <<4:16/little, 0:32/little, 8:8>>,
        CUBody = <<CUHeader/binary, Die/binary>>,
        UnitLen = byte_size(CUBody),
        DebugInfo = <<UnitLen:32/little, CUBody/binary>>,
...
make_debug_line_v4() ->
    ...
    HeaderBody = <<1:8, 1:8, 1:8, (256-5):8, 14:8, 13:8,
                   StdOpcLens/binary,
                   IncDirs/binary,
                   FileNames/binary>>,
    ...
    LineProgram = <<0, 1, 1>>,
    UnitBody = <<4:16/little, HeaderLen:32/little,
                 HeaderBody/binary, LineProgram/binary>>,
