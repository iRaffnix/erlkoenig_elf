...
find_at_addr(_Addr, []) ->
    error;
find_at_addr(Addr, [S = #elf_sym{value = V, size = Sz} | _])
  when Sz > 0, Addr >= V, Addr < V + Sz ->
    {ok, S};
...
parse_syms_1(<<>>, _StrData, _Endian, Acc) ->
    lists:reverse(Acc);
parse_syms_1(<<StName:32/little, StInfo:8, _StOther:8,
...
parse_syms_1(<<StName:32/big, StInfo:8, _StOther:8,
               StShndx:16/big, StValue:64/big, StSize:64/big,
               Rest/binary>>,
             StrData, big, Acc) ->
    Sym = make_sym(StName, StInfo, StShndx, StValue, StSize, StrData),
