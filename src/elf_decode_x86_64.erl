%%%-------------------------------------------------------------------
%%% @doc Minimal x86-64 instruction decoder for syscall extraction.
%%%
...
-type insn_type() :: syscall | mov_rax_imm | xor_rax_rax
                   | ret | call | jmp | jcc | other.
...
%% @doc Scan backwards from a SYSCALL site to resolve the RAX value.
...
resolve_syscall(_Bin, SyscallOffset, Insns) ->
    ...
    Before = lists:reverse([I || I <- Insns,
                            I#x86_insn.offset < SyscallOffset]),
    scan_for_rax(Before).

%% @doc Extract all syscall numbers from a code section.
...
scan_for_rax([#x86_insn{type = T} | _])
  when T =:= ret; T =:= call; T =:= jmp; T =:= jcc; T =:= syscall ->
    unresolved;
scan_for_rax([_ | Rest]) ->
    scan_for_rax(Rest).
...
decode_insn(Bytes, Off) ->
    ...
    case Rest0 of
        ...
        <<16#0F, 16#05, _/binary>> ->
            {ok, #x86_insn{offset = Off, length = PrefixLen + 2,
                           type = syscall, value = undefined}};
        ...
        %% RET (C3)
        <<16#C3, _/binary>> ->
            {ok, #x86_insn{offset = Off, length = PrefixLen + 1,
                           type = ret, value = undefined}};

        %% CALL rel32 (E8)
        <<16#E8, _:32/little, _/binary>> ->
            {ok, #x86_insn{offset = Off, length = PrefixLen + 5,
                           type = call, value = undefined}};

        %% JMP rel32 (E9)
        <<16#E9, _:32/little, _/binary>> ->
            {ok, #x86_insn{offset = Off, length = PrefixLen + 5,
                           type = jmp, value = undefined}};
                ...
                true ->
                    ...
                    case Rest0 of
                        <<_:8, Imm:64/little-signed, _/binary>> ->
                            IsRax = (Reg =:= 0),
                            Type = case IsRax of true -> mov_rax_imm; false -> other end,
                            Val = case IsRax of true -> Imm; false -> undefined end,
                            {ok, #x86_insn{offset = Off, length = PrefixLen + 1 + 8,
                                           type = Type, value = Val}};
                        _ ->
                            {error, truncated}
                    end;
                false ->
                    case Rest0 of
                        <<_:8, Imm:32/little-signed, _/binary>> ->
                            ...
                            Type = case IsRax of true -> mov_rax_imm; false -> other end,
                            Val = case IsRax of true -> Imm; false -> undefined end,
                            {ok, #x86_insn{offset = Off, length = PrefixLen + 1 + 4,
                                           type = Type, value = Val}};
                        _ ->
                            {error, truncated}
                    end
        ...
        <<16#C7, ModRM, Tail2/binary>> ->
            ...
            case RegField of
                0 ->
                    ...
                    TotalLen = 2 + Extra + 4,  %% opcode + modrm + extras + imm32
                    Mod = ModRM bsr 6,
                    Rm = ModRM band 7,
                ...
                _ ->
                    Extra = modrm_extra_len(ModRM, Tail2),
                    ImmSize = case Op66 of true -> 2; false -> 4 end,
                    {ok, #x86_insn{offset = Off,
        ...
        <<16#C6, ModRM, Tail2/binary>> ->
            Extra = modrm_extra_len(ModRM, Tail2),
            {ok, #x86_insn{offset = Off, length = PrefixLen + 2 + Extra + 1,
                           type = other, value = undefined}};
        ...
        <<Opc, ModRM, Tail2/binary>> when
            ...
            (Opc band 16#C0 =:= 16#00 andalso Opc band 16#04 =:= 0
             andalso Opc =/= 16#0F andalso Opc =/= 16#06 andalso Opc =/= 16#07
             andalso Opc =/= 16#0E andalso Opc =/= 16#16 andalso Opc =/= 16#17
             andalso Opc =/= 16#1E andalso Opc =/= 16#1F
             andalso Opc =/= 16#26 andalso Opc =/= 16#27
             andalso Opc =/= 16#2E andalso Opc =/= 16#2F
             andalso Opc =/= 16#36 andalso Opc =/= 16#37
             andalso Opc =/= 16#3E andalso Opc =/= 16#3F);
            %% 84-8F: TEST, XCHG, MOV, LEA, etc.
            (Opc >= 16#84 andalso Opc =< 16#8F
             andalso Opc =/= 16#8C andalso Opc =/= 16#8E
             andalso Opc =/= 16#8D);  %% 8D = LEA, handled but has ModRM
            Opc =:= 16#8D;  %% LEA
            Opc =:= 16#8C;  %% MOV Sreg
            Opc =:= 16#8E;  %% MOV Sreg
            %% D0-D3: shift group
        ...
        ->
            ...
            {ok, #x86_insn{offset = Off, length = PrefixLen + 2 + Extra + ImmSize,
        ...
        <<Opc, _/binary>> when Opc band 16#C7 =:= 16#05 ->
            ...
            case Rest0 of
                <<_:8, _:ImmSize/binary, _/binary>> ->
                    {ok, #x86_insn{offset = Off, length = PrefixLen + 1 + ImmSize,
                                   type = other, value = undefined}};
                _ ->
                    {error, truncated}
            end;
        ...
        <<16#C2, _:16/little, _/binary>> ->
            {ok, #x86_insn{offset = Off, length = PrefixLen + 3,
                           type = ret, value = undefined}};
        ...
        <<Opc, _/binary>> when Opc >= 16#A4, Opc =< 16#A7;
                               Opc >= 16#AA, Opc =< 16#AF ->
            {ok, #x86_insn{offset = Off, length = PrefixLen + 1,
                           type = other, value = undefined}};
...
-spec decode_two_byte(byte(), binary(), non_neg_integer(),
                      non_neg_integer(), boolean()) ->
    {ok, #x86_insn{}} | {error, term()}.

%% Jcc rel32 (0F 80 - 0F 8F)
decode_two_byte(Sec, _Tail, Off, PLen, _Op66)
  when Sec >= 16#80, Sec =< 16#8F ->
    {ok, #x86_insn{offset = Off, length = PLen + 2 + 4,
                   type = jcc, value = undefined}};

%% SETcc (0F 90 - 0F 9F) — ModR/M
decode_two_byte(Sec, Tail, Off, PLen, _Op66)
  when Sec >= 16#90, Sec =< 16#9F ->
    case Tail of
        <<ModRM, Tail2/binary>> ->
            Extra = modrm_extra_len(ModRM, Tail2),
            {ok, #x86_insn{offset = Off, length = PLen + 3 + Extra,
                           type = other, value = undefined}};
        _ ->
            {error, truncated}
    end;
...
decode_two_byte(Sec, Tail, Off, PLen, _Op66)
  when Sec =:= 16#B6; Sec =:= 16#B7; Sec =:= 16#BE; Sec =:= 16#BF ->
    case Tail of
        <<ModRM, Tail2/binary>> ->
            Extra = modrm_extra_len(ModRM, Tail2),
            {ok, #x86_insn{offset = Off, length = PLen + 3 + Extra,
...
decode_two_byte(Sec, Tail, Off, PLen, _Op66)
  when Sec =:= 16#A3; Sec =:= 16#AB; Sec =:= 16#B3; Sec =:= 16#BB ->
    case Tail of
        <<ModRM, Tail2/binary>> ->
            Extra = modrm_extra_len(ModRM, Tail2),
            {ok, #x86_insn{offset = Off, length = PLen + 3 + Extra,
...
decode_two_byte(16#BA, Tail, Off, PLen, _Op66) ->
    case Tail of
        <<ModRM, Tail2/binary>> ->
            Extra = modrm_extra_len(ModRM, Tail2),
            {ok, #x86_insn{offset = Off, length = PLen + 3 + Extra + 1,
                           type = other, value = undefined}};
        _ ->
            {error, truncated}
    end;
...
decode_two_byte(16#38, Tail, Off, PLen, _Op66) ->
    case Tail of
        <<_ThirdByte, ModRM, Tail2/binary>> ->
            Extra = modrm_extra_len(ModRM, Tail2),
            {ok, #x86_insn{offset = Off, length = PLen + 4 + Extra,
                           type = other, value = undefined}};
        _ ->
            {error, truncated}
    end;
...
decode_two_byte(16#3A, Tail, Off, PLen, _Op66) ->
    case Tail of
        <<_ThirdByte, ModRM, Tail2/binary>> ->
            Extra = modrm_extra_len(ModRM, Tail2),
            {ok, #x86_insn{offset = Off, length = PLen + 4 + Extra + 1,
                           type = other, value = undefined}};
        _ ->
            {error, truncated}
    end;
...
decode_two_byte(Sec, _Tail, Off, PLen, _Op66)
  when Sec =:= 16#31; Sec =:= 16#A2 ->
    {ok, #x86_insn{offset = Off, length = PLen + 2,
                   type = other, value = undefined}};

%% WRMSR/RDMSR/RDPMC (0F 30, 0F 32, 0F 33)
decode_two_byte(Sec, _Tail, Off, PLen, _Op66)
  when Sec =:= 16#30; Sec =:= 16#32; Sec =:= 16#33 ->
    {ok, #x86_insn{offset = Off, length = PLen + 2,
                   type = other, value = undefined}};
...
decode_two_byte(Sec, Tail, Off, PLen, _Op66)
  when (Sec >= 16#10 andalso Sec =< 16#17);  %% MOVxPS/MOVxPD etc.
       (Sec >= 16#28 andalso Sec =< 16#2F);  %% MOVAPS, COMISS, etc.
       (Sec >= 16#50 andalso Sec =< 16#7F);  %% SSE arith/logic
       (Sec >= 16#A0 andalso Sec =< 16#A1);  %% PUSH/POP FS/GS
       Sec =:= 16#AE;                         %% LFENCE etc (ModR/M)
       (Sec >= 16#C2 andalso Sec =< 16#C6);  %% SSE compare, shuffle
       (Sec >= 16#D0 andalso Sec =< 16#FF) -> %% SSE misc
    case Tail of
        <<ModRM, Tail2/binary>> ->
            Extra = modrm_extra_len(ModRM, Tail2),
            ImmSize = case Sec of
                16#70 -> 1;  %% PSHUFx imm8
                16#C2 -> 1;  %% CMPxPS/PD imm8
                16#C4 -> 1;  %% PINSRW imm8
                16#C5 -> 0;  %% PEXTRW (no imm, despite reg encoding)
                16#C6 -> 1;  %% SHUFPS/PD imm8
                _     -> 0
            end,
            {ok, #x86_insn{offset = Off, length = PLen + 3 + Extra + ImmSize,
                           type = other, value = undefined}};
        _ ->
            {error, truncated}
    end;
...
eat_prefixes(<<Rex, Rest/binary>>, Acc)
  when Rex >= 16#40, Rex =< 16#4F ->
...
modrm_extra_len(ModRM, _Tail) ->
    ...
    case Mod of
        ...
        2#00 ->
            case Rm of
                2#100 -> 1 + sib_disp_len(2#00, _Tail);  %% SIB follows
                2#101 -> 4;  %% RIP-relative disp32
                _     -> 0
            end;
        2#01 ->
            case Rm of
                2#100 -> 1 + 1 + sib_base_extra(2#01, _Tail);  %% SIB + disp8
                _     -> 1   %% disp8
            end;
        2#10 ->
            case Rm of
                2#100 -> 1 + 4 + sib_base_extra(2#10, _Tail);  %% SIB + disp32
                _     -> 4   %% disp32
            end
    end.
...
modrm_opcode_imm(16#F7, ModRM, Op66) ->
    ...
    Reg = (ModRM bsr 3) band 7,
    case Reg of
        0 -> case Op66 of true -> 2; false -> 4 end;
        1 -> case Op66 of true -> 2; false -> 4 end;
        _ -> 0
    end;
...
-spec moffs_insn(binary(), non_neg_integer(), non_neg_integer(),
                 non_neg_integer()) ->
    {ok, #x86_insn{}} | {error, term()}.
moffs_insn(Rest, Off, PLen, _OpcLen) ->
    ...
    case Rest of
        <<_:8, _:64, _/binary>> ->
            {ok, #x86_insn{offset = Off, length = PLen + 1 + 8,
                           type = other, value = undefined}};
        _ ->
            {error, truncated}
    end.
