%%% @doc Dependency analysis and anomaly detection for ELF binaries.
%%%
...
%% @doc Detect anomalies using default expectations.
...
%% @doc Detect anomalies: packages with unexpected capabilities.
...
deps_rust(Elf) ->
    case elf_lang_rust:parse(Elf) of
        {ok, #rust_info{crates = Crates}} ->
            Normalized = [#{name => C#rust_crate.name,
...
-spec capabilities_go(#elf{}) ->
    ...
    {error, term()}.
capabilities_go(Elf) ->
    case elf_lang_go:function_package_map(Elf) of
        {ok, PkgMap} ->
            case elf_syscall:extract(Elf) of
                {ok, #{sites := Sites, resolved := Resolved, arch := Arch}} ->
...
build_func_ranges_1([#go_func{entry = Entry, package = Pkg},
                     #go_func{entry = NextEntry} = Next | Rest]) ->
...
assign_sites_to_packages(Sites, Ranges, Resolved, _Arch) ->
    lists:foldl(
        fun(#{addr := Addr, syscall_nr := Nr}, Acc) when is_integer(Nr) ->
...
-spec find_package(non_neg_integer(),
                   [{non_neg_integer(), non_neg_integer(), binary()}]) ->
    {ok, binary()} | error.
find_package(_Addr, []) ->
    error;
...
-spec find_package_bs(non_neg_integer(),
                      [{non_neg_integer(), non_neg_integer(), binary()}],
        ...
        fun(Pkg, #{categories := ActualCats, syscalls := Syscalls}, Acc) ->
            ...
            case Unexpected of
                ...
                _ ->
                    ...
                    UnexpectedSyscalls = [S || S <- Syscalls,
                                               lists:member(
                                                   elf_syscall_db:category(S),
                                                   Unexpected)],
                    [#{package => Pkg,
                       unexpected => lists:sort(Unexpected),
                       syscalls => lists:usort(UnexpectedSyscalls)} | Acc]
            end
        end,
        [],
        Caps).
