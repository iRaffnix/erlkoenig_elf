%%% @doc ELF64 binary parser in pure Erlang.
%%%
...
-type elf() :: #elf{}.
-type parse_error() :: {error, not_elf}
                     | {error, not_elf64}
...
section_data(#elf_shdr{type = nobits}, _Elf) ->
    {error, nobits};
section_data(#elf_shdr{offset = Off, size = Sz}, #elf{bin = Bin})
  when Off + Sz =< byte_size(Bin) ->
...
%% @doc True if the binary is a position-independent executable
...
parse_elf(Bin, Endian) ->
    case parse_header(Bin, Endian) of
        {ok, Hdr} ->
            case parse_phdrs(Bin, Endian, Hdr) of
                {ok, Phdrs} ->
                    case parse_shdrs(Bin, Endian, Hdr) of
                        {ok, Shdrs0} ->
...
parse_header(Bin, little) ->
    case Bin of
        <<16#7F, "ELF",
          ...
          Flags:32/little,
          _EhSize:16/little,
          _PhEntSize:16/little,
          PhNum:16/little,
          _ShEntSize:16/little,
          ShNum:16/little,
          ShStrNdx:16/little,
          _/binary>> ->
            {ok, #elf_header{
...
vaddr_to_offset_1(_Vaddr, []) ->
    {error, not_mapped};
vaddr_to_offset_1(Vaddr, [#elf_phdr{type = load,
                                     vaddr = Base,
                                     offset = Off,
                                     memsz = Memsz} | _])
  when Vaddr >= Base, Vaddr < Base + Memsz ->
    {ok, Off + (Vaddr - Base)};
...
-spec decode_phdr_type(non_neg_integer()) ->
    load | dynamic | interp | note | tls | phdr
...
decode_phdr_flags(Flags) ->
    lists:append([
        case Flags band ?PF_R of 0 -> []; _ -> [r] end,
        case Flags band ?PF_W of 0 -> []; _ -> [w] end,
        case Flags band ?PF_X of 0 -> []; _ -> [x] end
    ]).
...
decode_shdr_flags(Flags) ->
    lists:append([
        case Flags band ?SHF_WRITE     of 0 -> []; _ -> [write]     end,
        case Flags band ?SHF_ALLOC     of 0 -> []; _ -> [alloc]     end,
        case Flags band ?SHF_EXECINSTR of 0 -> []; _ -> [execinstr] end,
        case Flags band ?SHF_MERGE     of 0 -> []; _ -> [merge]     end,
        case Flags band ?SHF_STRINGS   of 0 -> []; _ -> [strings]   end,
        case Flags band ?SHF_TLS       of 0 -> []; _ -> [tls]       end
    ]).
