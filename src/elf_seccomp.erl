%%% @doc Seccomp-BPF profile generator from ELF syscall analysis.
%%%
...
-export([from_binary/1, from_syscalls/2,
         to_json/1, to_bpf/1, to_erlang/1,
         analyze_to_json/1]).
...
from_binary(#elf{} = Elf) ->
    case elf_syscall:extract(Elf) of
        {ok, #{arch := Arch, resolved := Resolved, unresolved_count := Unresolved}} ->
            ...
            Names = ordsets:from_list([N || N <- maps:values(Resolved),
                                            N =/= <<"unknown">>]),
            {ok, #seccomp_profile{
                arch = Arch,
...
to_erlang(#seccomp_profile{arch = Arch, default_action = Action,
                           syscalls = Syscalls, names = Names,
                           unresolved = Unresolved}) ->
    #{
        arch => Arch,
...
bpf_bst(Sorted) ->
    ...
    AllowIdx = TotalLen - 1,  % 0-based index of ALLOW instruction
...
bst_build_range(Sorted) ->
    N = length(Sorted),
    Mid = (N + 1) div 2,  %% 1-based median position
...
-spec bpf_jump(non_neg_integer(), non_neg_integer(),
