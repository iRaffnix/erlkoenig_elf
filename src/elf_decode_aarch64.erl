%%%-------------------------------------------------------------------
%%% @doc Minimal AArch64 instruction decoder for syscall extraction.
%%%
...
-type insn_type() :: svc | mov_x8_imm | movk_x8_imm
                   | ret | bl | b | b_cond | other.
...
%% @doc Scan backwards from an SVC site to resolve the X8 value.
...
scan_for_x8([#aarch64_insn{type = T} | _], _, _)
  when T =:= ret; T =:= bl; T =:= b; T =:= b_cond; T =:= svc ->
    unresolved;
...
classify(Insn, Offset) ->
    case classify_type(Insn) of
        {Type, Value} ->
            #aarch64_insn{offset = Offset, length = 4,
        ...
        true ->
            ...
            case Fixed of
                2#100101 ->
                    ...
                    case Opc of
                        2#10 -> {mov_x8_imm, ShiftedVal};   %% MOVZ
                        2#11 -> {movk_x8_imm, ShiftedVal};  %% MOVK
                        _ -> other
                    end;
                _ ->
                    other
